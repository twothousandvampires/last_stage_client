...
export default class Render {
    ...
    draw(inputs) {
        ...
        for (let i = 0; i < to_draw.length; i++) {
            ...
            if (elem.by_centr) {
                this.ctx.drawImage(this.data.map.get(elem.sprite_name), elem.frame * elem.sprite_w, elem.y_frame_offset + 1, elem.sprite_w, elem.sprite_h - 1, rel_x - elem.sprite_w / (2 * this.downscale), rel_y - (elem.sprite_h / (2 * this.downscale)) - elem.z, elem.sprite_w / this.downscale, elem.sprite_h / this.downscale);
            }
            else {
                this.ctx.drawImage(this.data.map.get(elem.sprite_name), elem.frame * elem.sprite_w, elem.y_frame_offset + 1, elem.sprite_w, elem.sprite_h - 1, rel_x - elem.sprite_w / (2 * this.downscale), rel_y - ((elem.sprite_h / this.downscale) - 1) - elem.z, elem.sprite_w / this.downscale, elem.sprite_h / this.downscale);
            }
            if (elem.flipped) {
                this.ctx.restore();
            }
